...
from mesh import nodes_I, Nodes2D, xytors, rstoab, Nodes3D, xyztorst, rsttoabc
...
def JacobiP(x,alpha,beta,N):
        ...
        if (N==0):
            ...
            PL[1,:] = np.squeeze(((alpha+beta+2)*xp/2 + (alpha-beta)/2)/math.sqrt(gamma1))
            if (N==1):
                P=np.transpose(PL[N,:])
            else:
                ...
                for i in range(1,N):
                    ...
                    PL[i+1,:] = (1/anew*( -aold*PL[i-1,:] + (np.squeeze(xp)-bnew)*PL[i,:]))
                    aold =anew
...
if (dim==2):
    ...
    def Lift2D(V, r, s):
        ...
        Emat = np.zeros((PolyDeg, Nfaces*Nb))
...
################################################################################################################# Dim=3 ############################################################################################################################
    ...
    def GradVandermonde3D(N,r,s,t):
        ...
        V3Dr = np.zeros((len(r),(N+1)*(N+2)*(N+3)/6))
        V3Ds = np.zeros((len(r),(N+1)*(N+2)*(N+3)/6))
        V3Dt = np.zeros((len(r),(N+1)*(N+2)*(N+3)/6))
    ...
    def Lift3D(V,R,S,T):
        ...
        for face in range(Nfaces):
            ...
            idc = (face-1)*Nb+np.arange(face)*Nb
    ...
    def matrices(nodes, elements, Ne):
        ...
        Vr, Vs, Vt = GradVandermonde2D(PolyDeg, r, s, t)
        Drw = np.matmul(np.matmul(V,np.transpose(Vr)),np.linalg.inv(np.matmul(V,np.transpose(V))))
        Dsw = np.matmul(np.matmul(V,np.transpose(Vs)),np.linalg.inv(np.matmul(V,np.transpose(V))))
